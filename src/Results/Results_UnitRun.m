function [downwardRate, upwardRate, records] = ...
    Results_UnitRun(stimList, constList, paramList, rounds, records, deltaT, mode, attention, integration)
% Run the simulation for certain rounds.

    % Adapted from the function used in Network Simulation.

    % Change the integration inhibition level into integration inhibition
    % strength
    integration = 1 + (integration - 1) / 10;
    ...
    W_AM = paramList(1,3) / integration; W_MA = paramList(1,4) / integration;
    W_VM = paramList(1,5) / integration; W_MV = paramList(1,6) / integration;
    
    if mode == 'A'
        decayRateV = decayRate / attention;
        decayRateA = decayRate * attention;
        decayRateSC = decayRate;
    else
        decayRateV = decayRate * attention;
        decayRateA = decayRate / attention;
        decayRateSC = decayRate;
    ...
    for i=1:1:rounds
        deltaV_D = ReynoldsEquation(decayRateV, maxFiringRate,...
        ...
        deltaV_U = ReynoldsEquation(decayRateV, maxFiringRate,...
        ...
        deltaA_D = ReynoldsEquation(decayRateA, maxFiringRate,...
        ...
        deltaA_U = ReynoldsEquation(decayRateA, maxFiringRate,...
        ...
        deltaSC_D = ReynoldsEquation(decayRateSC, maxFiringRate,...
        ...
        deltaSC_U = ReynoldsEquation(decayRateSC, maxFiringRate,...
            ( W_VM * (records(2, start) - baseline ) / maxFiringRate +...
                W_AM * (records(4, start) - baseline ) / maxFiringRate),...
            ( W_VM * (records(1, start) - baseline) / maxFiringRate + ...
                W_AM * (records(3, start) - baseline) / maxFiringRate),...
